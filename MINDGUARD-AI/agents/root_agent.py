...
# from agents.response_agent import response_agent
# from agents.resource_agent import resource_agent
# from security import safety_check, safety_response
...
#         # 0. Deterministic crisis check on the raw user message, BEFORE any
#         #    LLM sub-agent runs. This can't be talked out of firing by
#         #    prompt phrasing, doesn't depend on the model noticing anything,
#         #    and skips straight to a hard-coded safe response -- it does
#         #    NOT continue on into checkin/analysis/response/resource this
#         #    turn, since those aren't equipped to handle a crisis message.
#         user_text = ""
...
#                     role="model",
#                     parts=[types.Part(text=safety_response())],
#                 ),
#             )
#             return
...
class MindGuardRootAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        # 0. Deterministic crisis check on the raw user message, BEFORE any
        #    LLM sub-agent runs. Runs on EVERY turn, not just HIGH risk --
        #    it can't be talked out of firing by prompt phrasing, doesn't
        #    depend on the model noticing anything, and skips straight to
        #    a hard-coded safe response if triggered.
        user_text = ""
        if ctx.user_content and ctx.user_content.parts:
            user_text = ctx.user_content.parts[0].text or ""
